...
export function CompanyGreeting() {
  ...
  const bodyLead =
    (typeof body.lead === "string" && body.lead) || "안녕하십니까. 농업법인 (주)산골 대표이사 정현철입니다.";
  const bodyParagraphs = Array.isArray(body.paragraphs)
    ? (body.paragraphs as string[]).filter((p) => typeof p === "string" && p.trim())
    : [
  ...
  const closingThanks = (typeof body.closingThanks === "string" && body.closingThanks) || "감사합니다.";
  const closingSign =
    (typeof body.closingSign === "string" && body.closingSign) || "농업 법인회사 (주)산골 임직원 일동";
      ...
      <div className="site-container py-16 md:py-24 w-full flex flex-col md:flex-row gap-10 md:gap-12 lg:gap-16 items-start">
        ...
        <div className="w-full flex-1 min-w-0 flex flex-col justify-center">
          ...
          <div className="space-y-6 text-[#4F6F52] text-[17px] leading-[1.8] font-medium tracking-wide">
            <p className="text-xl font-bold text-[#1A4D2E]">{bodyLead}</p>
            {bodyParagraphs.map((paragraph) => (
              <p key={paragraph.slice(0, 24)}>{paragraph}</p>
            ))}
            <div className="pt-10 text-right">
              <p className="text-gray-500 mb-2">{closingThanks}</p>
              <p className="text-xl font-bold text-[#1A4D2E]">{closingSign}</p>
            </div>
          </div>
        </div>
